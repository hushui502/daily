...
type RequestPreparer func(r *http.Request)
...
// If dest and dest.dlstate file both exist, it will attempt to resume the download.
// It will maintain a dest.dlstate file to track the parts of the file that have been downloaded.
...
func Get(dest, source string, opts *Options) (s <-chan State, err error) {
	...
	var pending []Block
	for offset := int64(0); offset < size; offset += o.BlockSize {
		length := o.BlockSize
		if offset+length > size {
			length = size - offset
		}
		pending = append(pending, Block{Offset: offset, Length: length})
	}
...
func (o *Options) getSize(url string) (int64, error) {
	...
	if err != nil {
		return 0, fmt.Errorf("roudtrip error: %v", err)
	}

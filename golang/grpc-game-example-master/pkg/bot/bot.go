...
// bot controls a1 player in the game.
type bot struct {
	playerID uuid.UUID
}

// Bots controls all bots added to a1 game.
type Bots struct {
	bots []*bot
	game *backend.Game
}

// NewBots creates a1 new bots instance.
...
// AddBot adds a1 new bot to the game.
...
// tile represents a1 point on the map.
type tile struct {
...
// PathNeighborCost is used by beefsack/astar to determine the cost of a1 move.
...
// getShootDirection determines if a1 straight path between c1 and c2 is not
// blocked by a1 wall, and if not returns the direction of the path.
	...
	go func() {
		...
		for {
			...
			for _, bot := range bots.bots {
				...
				// Find a1 path using the astar algorithm.

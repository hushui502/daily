...
func TestAof(t *testing.T) {
	...
	for i := 0; i < size; i++ {
		...
		cursor++
		Set(aofWriteDB, utils.ToBytesList(key, utils.RandString(8), "EX", "10000"))
		keys = append(keys, key)
	}
	for i := 0; i < size; i++ {
		key := strconv.Itoa(cursor)
		cursor++
		RPush(aofWriteDB, utils.ToBytesList(key, utils.RandString(8)))
		keys = append(keys, key)
	}
	for i := 0; i < size; i++ {
		key := strconv.Itoa(cursor)
		cursor++
		HSet(aofWriteDB, utils.ToBytesList(key, utils.RandString(8), utils.RandString(8)))
		keys = append(keys, key)
	}
	for i := 0; i < size; i++ {
		key := strconv.Itoa(cursor)
		cursor++
		SAdd(aofWriteDB, utils.ToBytesList(key, utils.RandString(8)))
		keys = append(keys, key)
	}
	for i := 0; i < size; i++ {
		key := strconv.Itoa(cursor)
		cursor++
		ZAdd(aofWriteDB, utils.ToBytesList(key, "10", utils.RandString(8)))
		keys = append(keys, key)
	}
...
func TestRewriteAOF(t *testing.T) {
	...
	for i := 0; i < size; i++ {
		...
		cursor++
		Set(aofWriteDB, utils.ToBytesList(key, utils.RandString(8)))
		Set(aofWriteDB, utils.ToBytesList(key, utils.RandString(8)))
		keys = append(keys, key)
	}
	// test ttl
	for i := 0; i < size; i++ {
		key := "str" + strconv.Itoa(cursor)
		cursor++
		Set(aofWriteDB, utils.ToBytesList(key, utils.RandString(8), "EX", "1000"))
	...
	for i := 0; i < size; i++ {
		...
		HSet(aofWriteDB, utils.ToBytesList(key, field, utils.RandString(8)))
		HSet(aofWriteDB, utils.ToBytesList(key, field, utils.RandString(8)))
		keys = append(keys, key)
	}
	for i := 0; i < size; i++ {
		...
		SAdd(aofWriteDB, utils.ToBytesList(key, member))
		SAdd(aofWriteDB, utils.ToBytesList(key, member))
		keys = append(keys, key)
	}
	for i := 0; i < size; i++ {
	...
	for _, key := range ttlKeys {
		ret := TTL(aofReadDB, utils.ToBytesList(key))

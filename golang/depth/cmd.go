...
// parse constructs a Tree from command-line arguments, and returns the
// remaining user-supplied package names
func parse(args []string) (*Tree, []string) {
...
func handlePkgs(t *Tree, pkgs []string, outputJSON bool, explainPkg string) error {
...
func writePkgSummary(w io.Writer, pkg Pkg) {
	var sum summary
...
func collectSummary(sum *summary, pkg Pkg, nameSet map[string]struct{}) {
...
func writePkgJSON(w io.Writer, p Pkg) {
	e := json.NewEncoder(w)
	e.SetIndent("", "  ")
	e.Encode(p)
}

func writePkg(w io.Writer, p Pkg) {
	fmt.Fprintf(w, "%s\n", p.String())
...
func writePkgRec(w io.Writer, p Pkg, closed []bool, isLast bool) {
	var prefix string
...
func writeExplain(w io.Writer, pkg Pkg, stack []string, explain string) {

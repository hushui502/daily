...
func main() {
	...
	router.Static("/api/v1/code2img/data/image", "./data/images")
...
func code2img(c *gin.Context) {
	...
	err = render("./data/image/" + id + ".png", b.Code)
	if err != nil {
		c.String(http.StatusBadRequest, fmt.Sprintf("Error: %s", err))
		return
	}
...
func render(imgfile, code string) error {
	...
	err := chromedp.Run(ctx, chromedp.Tasks{
		chromedp.EmulateViewport(2560, 1400),
...
func screenshot(sel interface{}, picbuf *[]byte, opts ...chromedp.QueryOption) chromedp.QueryAction {
	...
	return chromedp.QueryAfter(sel, func(ctx context.Context, rt runtime.ExecutionContextID, nodes ...*cdp.Node) error {
		if len(nodes) < 1 {

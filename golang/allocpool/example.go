...
func ExampleNew_customBlockSize() {
	pool, err := New(2, WithBlockSize(8*1024*1024))
	if err != nil {
		panic(err)
	}
	defer pool.Close()
...
func ExamplePool_AllocCount() {
	...
	newPool, err := New(3, WithPreAlloc(2))
	if err != nil {
		panic(err)
	}
	fmt.Printf("total allocated: %d\n", newPool.AllocCount())

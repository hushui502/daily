...
type locVarInfo struct {
	...
	slot int		// 字段记录与局部变量名绑定的寄存器所以
	captured bool	// 局部变量是否被闭包捕获
}

// 闭包绑定的外围局部变量
// 由于只能绑定一个唯一的Upvalue，所以不需要链结构
type upvalInfo struct {
	locVarSlot int		// 该局部变量所占的寄存器索引
	upvalIndex int		// 直接外围函数Upvalue表中的索引
	index int			// Upvalue在函数中出现的顺序
}

type funcInfo struct {
	constants map[interface{}]int	// k 常量值 v是常量值在常量表中的索引
	usedRegs int	// 使用的寄存器数目
	maxRegs int		// 当前最大的寄存器数目
	scopeLv int		// 当前作用域层次，作用域从0开始，每进一个作用域+1
...
/*	registers */
// 分配一个寄存器，必要时更新最大的寄存器数量，并返回寄存器索引
...
func (self *funcInfo) enterScope(breakable bool) {
	self.scopeLv++
	if breakable {
		self.breaks = append(self.breaks, []int{})	// 循环块	数组内记录待处理的跳转指令
	} else {
...
// 根据退出作用域，删除该作用域内的局部变量（解绑局部变量名，回收寄存器）
func (self *funcInfo) removeLocVar(locVar *locVarInfo) {
	self.freeReg()	// 先回收寄存器
	if locVar.prev == nil {
		delete(self.locNames, locVar.name)
	} else if locVar.prev.scopeLv == locVar.scopeLv {	// 同名变量在同一作用域 ==> 递归
...
// 检查局部变量是否已经和某个寄存器绑定，如果是则返回寄存器索引，否则返回-1
...
/* upvalues */
// 判断名字是否已经和Upvalues绑定，如果是则返回索引，否则尝试绑定
...
// 当退出作用域时，将处于开启状态的Upvalue闭合。
// 如果需要处理局部变量，该方法h会产生一条JMP指令，将操作数A给出需要处理的第一个局部变量
...
/* code */
// 该方法返回已经生成的最后一条指令的程序计数器，也就是该指令的索引

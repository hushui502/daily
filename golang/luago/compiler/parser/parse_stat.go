...
stat ::=  ‘;’
	...
	| functioncall
*/
// 通过前瞻一个token我们可以锁定其中的13种语句
// 对于局部变量声明还是局部函数定于，需要前瞻2个token
// 对于for循环，需要前瞻3个才能确定解析数值for还是统用for循环
// 剩下的函数调用和赋值语句
...
func parseBreakStat(lexer *Lexer) *BreakStat {
	...
	return &BreakStat{lexer.Line()}		// line number
}
...
func parseIfStat(lexer *Lexer) *IfStat {
	...
	for lexer.LookAhead() == TOKEN_KW_ELSEIF {	// else if
		lexer.NextToken()
...
func _finishForInStat(lexer *Lexer, name string) *ForInStat {
	nameList := _finishNameList(lexer, name)		// for namelist
...
func parseLocalAssignOrFuncDefStat(lexer *Lexer) Stat {
	...
	if lexer.LookAhead() == TOKEN_KW_FUNCTION {
		...
		return _finishLocalVarDefStat(lexer)
	}
}
...
func _finishLocalFuncDefStat(lexer *Lexer) *LocalFuncDefStat {
	...
	_, name := lexer.NextIdentifier()	// Name
	fdExp := parseFuncDefExp(lexer)		// function body

	return &LocalFuncDefStat{name, fdExp}
}

// local namelist [‘=’ explist]
func _finishLocalVarDefStat(lexer *Lexer) *LocalVarDeclStat {
	_, name := lexer.NextIdentifier() // local name
	nameList := _finishNameList(lexer, name) // {, name}
	var expList []Exp = nil
	if lexer.LookAhead() == TOKEN_OP_ASSIGN {
		lexer.NextToken()		// =
		expList = parseExpList(lexer)	// explist
...
func parseFuncDefStat(lexer *Lexer) *AssignStat {
	...
	if hasColon {		// var:name(args) => v.name(self, arg)

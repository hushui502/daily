...
func (t *SimpleChaincode) createUser(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	home := Home{Address:address, Energy:energy, Money:money, Status:1, PriKey:priKey, PubKey:pubKey}
...
func buyByAddress(stub shim.ChaincodeStubInterface, args []string) ([]byte, error) {
	...
	transaction := Transaction{BuyerAddress:args[2], BuyerAddressSign:args[1], SellerAddress:args[0], Energy:buyValue, Money:buyValue, Time:time.Now().Unix()}

...
func rebalance(a *App, req RebalanceRequest) bool {
	...
	// find the volume that are real
	rvolumes := make([]string, 0)
	for _, rv := range a.volumes {
	...
	if len(rvolumes) == 0 {
		fmt.Printf("can not rebalance, %s is missing!\n", string(req.key))
		return false
	}
	...
	// debug
	fmt.Println("rebalancing", string(req.key), rvolumes, "to", req.kvolumes)
	...
	// delete from the volumes that now are not kvolumes
	delete_error := false
	for _, v2 := range rvolumes {
		needs_delete := true

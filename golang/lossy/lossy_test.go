...
func TestBandwidth(t *testing.T) {
	...
	const headerOverhead = UDPv4MinHeaderOverhead
	...
	c = NewConn(c, bandwidth, 0, 0, 0, headerOverhead)
	pc = NewPacketConn(pc, bandwidth, 0, 0, 0, headerOverhead)
...
func TestLatency(t *testing.T) {
	...
	c = NewConn(c, 0, minLatency, maxLatency, 0, 0)
	pc = NewPacketConn(pc, 0, minLatency, maxLatency, 0, 0)
	var messages [][]byte
	for i := 0; i < messageCount; i++ {
		message := make([]byte, messageSize)
		rand.Read(message)
...
func TestPacketLoss(t *testing.T) {
	...
	c = NewConn(c, 0, 0, 0, packetLossRate, 0)
	pc = NewPacketConn(pc, 0, 0, 0, packetLossRate, 0)
	var messages [][]byte
	for i := 0; i < messageCount; i++ {
		message := make([]byte, messageSize)
		rand.Read(message)
...
func TestErrors(t *testing.T) {
	...
	pc = NewPacketConn(pc, 0, 0, 0, 0, 0)
	c = NewConn(c, 0, 0, 0, 0, 0)

...
/// Deploys the `briber.sol.sol` contract.
pub async fn deploy(opts: DeployOpts) -> color_eyre::Result<Address> {
    ...
    // compile the briber.sol contract each time
    let solc = ethers::utils::Solc::new("briber.sol.sol").build()?;
    ...
    if let Some(bribe) = opts.flashbots.bribe {
        ...
        match opts.flashbots.bribe_receiver {
            Some(bribe_receiver) => {
                ...
                // briber.sol.sol has a different method call depending on erc1155 or 721s
                // being sniped
